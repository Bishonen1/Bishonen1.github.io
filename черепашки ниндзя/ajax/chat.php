...
class Chat{
    ...
    return '<span style="color:#f00">Запрещено вводить ссылки на сайты!!!</span>';
}
	    ...
	    $this->db->Query('UPDATE db_users_b SET money_b = money_b+0 WHERE `user` = "'.$_SESSION["user"].'"');
		...
		$this->db->Query('UPDATE db_users_b SET money_b = money_b+0 WHERE `id` = "'.$refid.'"');
  ...
  if($moderresss == 1)
  {$str .= '<span class="time">('.date('H:i', $row['time']).')</span> <span class="user" style="color:#f00">'.htmlspecialchars($row['user']).'</span>: <font color="black">'.$this->bb_code($comm).'</font>';}
  if($moderresss != 1)
  {$str .= '<span class="time">('.date('H:i', $row['time']).')</span> <span class="user">'.htmlspecialchars($row['user']).'</span>: <font color="black">'.$this->bb_code($comm).'</font>';}
