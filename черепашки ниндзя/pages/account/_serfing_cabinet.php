...
?>
<div class="s-bk-lf">
	<div class="acc-title">Мои ссылки в серфинге</div>
</div>
<div class="silver-bk">
<div class="clr"></div>
...
Все добавленные ссылки проходят дополнительную проверку.<br>
После проверки Вашей ссылки вы можете пополнить рекламный бюджет и запустить рекламу.<br>
Проверка ссылки может проходить от 10 минут и до 12 часов, если надо срочно, пишите на почту: <font style="color:#FF0000;"><b>Ваш@mail.ru</b></font><br><br>
...
<span class="serfinfotext">№ <?php echo $row['id']; ?>&nbsp;&nbsp;Клик: <?php echo $row['price']; ?> золота.&nbsp;&nbsp;Просмотров: 
...
<input name="pay_adv" value="<?php echo $row['id']; ?>" type="hidden">Укажите сколько золота, вы хотите внести в бюджет рекламной площадки<br>(Минимум <span id="minsum<?php echo $row['id']; ?>"><?php echo $row['price']; ?></span> золото)<input name="pay_order" maxlength="10" value="<?php echo number_format($row['price']*1000, 2, '.', ''); ?>" type="text"><center><span class="button-red" title="Внести средства в бюджет площадки" onclick="javascript:submitform(<?php echo $row['id']; ?>);">Оплатить</span></center></form>
...
{
echo '<div class="alert" id="error"><b style="font-size: 14px;">Ссылок нет!</b></div>';
}
?>
